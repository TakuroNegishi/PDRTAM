...
//	mData.D.at<double>(1, 0) = -0.49801036600932364;
//	mData.D.at<double>(2, 0) = -0.0072060262085640288;
//	mData.D.at<double>(3, 0) = 0.011486207938787869;
//	mData.D.at<double>(4, 0) = 0.45905509098239838;

//	mData.D.at<double>(0, 0) = 0.217790112;
//	mData.D.at<double>(1, 0) = -1.14441943;
//	mData.D.at<double>(2, 0) = 0.000883583038;
//	mData.D.at<double>(3, 0) = 0.00457408885;
//	mData.D.at<double>(4, 0) = 0.0;
...
void CATAM::onceProc(cv::Mat &rgbaImg)
{
	// get color image
//	mCam.Get(mImg);
	// rgba -> bgr
//	cv::cvtColor(rgbaImg, mImg, cv::COLOR_BGRA2BGR);
	mImg = rgbaImg;
...
//	}
	...
	// TODO
	// show image
//	LOGE("onceProc() before draw()");
	draw(mImg);
//	LOGE("onceProc() after draw()");
...
void CATAM::drawProcess(cv::Mat &img) const
{
	...
	cv::putText(img, mText, cv::Point(0, img.rows - 5), cv::FONT_HERSHEY_SIMPLEX, 0.8, textcol, 2, cv::LINE_8, false);
...
void CATAM::setReset(void)
{
	isReset = true;
}

void CATAM::setStop(void)
{
	mState = STATE::STOP;
}

void CATAM::getPoint3Float (float *pointAry)
{
	std::vector<cv::Point3f> vvv = mData.map.GetAllPoint();
	for (int i = 0; i < vvv.size(); i++) {
		pointAry[(i*3) + 0] = vvv[i].x;
		pointAry[(i*3) + 1] = vvv[i].y;
		pointAry[(i*3) + 2] = vvv[i].z;

		__android_log_print(ANDROID_LOG_ERROR, __FILE__, "p[%d](%f, %f, %f)", i, vvv[i].x, vvv[i].y, vvv[i].z);
//		__android_log_print(ANDROID_LOG_ERROR, __FILE__, "y[%d]= %f \n", i, vvv[i].y);
//		__android_log_print(ANDROID_LOG_ERROR, __FILE__, "z[%d]= %f \n", i, vvv[i].z);
	}
}
...
//		pointAry[(i*3) + 2] = vvv[i].z;
		...
		ofs << vvv[i].x << "," << vvv[i].y << "," << vvv[i].z << std::endl;
...
//	LOGE("start drawView()");
	...
	cv::Mat mono = cv::Mat(img.size(), CV_8UC1);
	cv::Canny(kf.img, mono, 10, 100);

	std::vector<cv::Mat> color(4, mono);

	cv::Mat cimg = cv::Mat(img.size(), img.type());
	cv::merge(color, cimg);

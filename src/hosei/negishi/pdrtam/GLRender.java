...
//    	if (FileManager.write(header + ".txt", vertex))
//        	Log.e("", header + ".txt success!");
//    	else
//        	Log.e("", header + ".txt failed.");

//    	Log.e("", "vertexNum=" + originalVertex.length / 3 );
    	centering();
    }
...
*/
	...
	public void onDrawFrame(GL10 gl) {
		...
		v[0] = 0.0f; v[3] = 0.0f; v[1] = -one; v[4] = one;
		c[2] = 0.0f; c[1] = 1.0f;
		drawLine(gl, v, c); // 緑:Y軸
		v[1] = 0.0f; v[4] = 0.0f; v[2] = -one; v[5] = one;
		c[1] = 0.0f; c[0] = 1.0f;
		...
		drawPoints(gl, scaledVertex, c);
		
		// 最初の1点
//		float[] firstVertex = {scaledVertex[0], scaledVertex[1], scaledVertex[2]};
//		c[0] = 1.0f; c[1] = 0.0f; c[2] = 0.0f;
//		drawPoints(gl, firstVertex, c);
//		
//		// 2点目
//		float[] secondVertex = {scaledVertex[3], scaledVertex[4], scaledVertex[5]};
//		c[0] = 0.0f; c[1] = 1.0f; c[2] = 0.0f;
//		drawPoints(gl, secondVertex, c);
//		
//		// 3点目
//		float[] thirdVertex = {scaledVertex[6], scaledVertex[7], scaledVertex[8]};
...
//        };
		...
		for (int i = 0; i < triangleList.size(); i++) {
			...
			if (triangle.id1*3+2 >= scaledVertex.length || triangle.id2*3+2 >= scaledVertex.length || triangle.id3*3+2 >= scaledVertex.length) continue;
			...
			float[] vertices = {
					scaledVertex[triangle.id1*3], scaledVertex[triangle.id1*3+1], scaledVertex[triangle.id1*3+2],
					scaledVertex[triangle.id2*3], scaledVertex[triangle.id2*3+1], scaledVertex[triangle.id2*3+2],
					scaledVertex[triangle.id3*3], scaledVertex[triangle.id3*3+1], scaledVertex[triangle.id3*3+2]};
			// マッピング座標
			float[] coords = {
				(float) (triangle.p1.x / texWidth), (float) (triangle.p1.y / texHeight),
				(float) (triangle.p2.x / texWidth), (float) (triangle.p2.y / texHeight),
				(float) (triangle.p3.x / texWidth), (float) (triangle.p3.y / texHeight)
			};
//			Log.e("x1", ""+coords[0]);
//			Log.e("y1", ""+coords[1]);
//			Log.e("x2", ""+coords[2]);
...
//	        gl.glEnableClientState(GL10.GL_COLOR_ARRAY); // ポリゴン頂点色のバッファをセットしたことをOpenGLに伝える
    ...
    // テクスチャを描画するためのメソッド
    public void drawTexture(GL10 gl, float[] vertices, int textureID, float u, float v, float tex_w, float tex_h) {
    	// マッピング座標
        float[] coords = {
        	u, v + tex_h,			// 左下
        	u + tex_w, v + tex_h,	// 右下
...
//		float bottom = top + ((float)h / (float)textureImage.getHeight());
	...
	public void drawQuad(GL10 gl10, float leftTopX, float leftTopY, float leftTopZ, float leftBtmX, float leftBtmY, float leftBtmZ, 
			float rightTopX, float rightTopY, float rightTopZ, float rightBtmX, float rightBtmY, float rightBtmZ) {
		// ! 位置情報
		float positions[] = {
			// ! x y z
